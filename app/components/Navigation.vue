    ...
    <div class="container mx-auto px-4">
      <div class="flex justify-between items-center py-4">
        ...
        <nav class="hidden md:flex space-x-8 items-center">
          ...
          <NuxtLink to="/compare" class="text-luxury-green hover:text-gold transition-colors duration-300 dark:text-gray-200 dark:hover:text-gold relative">
          ...
          <NuxtLink to="/cart" class="relative text-luxury-green hover:text-gold transition-colors duration-300 dark:text-gray-200 dark:hover:text-gold">
            ...
            <span
              v-if="(cartStore.itemCount.value || cartStore.itemCount) > 0"
              class="absolute -top-2 -right-2 bg-gold text-luxury-green rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold animate-pop-in dark:bg-luxury-green dark:text-white"
            >
      ...
      <div v-if="mobileMenuOpen" class="md:hidden pb-4 transition-colors duration-300 dark:bg-gray-800">
        <nav class="flex flex-col space-y-3">
          ...
          <NuxtLink to="/compare" class="flex items-center text-luxury-green hover:text-gold transition-colors duration-300 py-2 dark:text-gray-200 dark:hover:text-gold relative">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            </svg>
            Compare
            <span v-if="comparisonCount > 0" class="absolute -top-2 -right-2 bg-gold text-luxury-green rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold animate-pop-in dark:bg-luxury-green dark:text-white">{{ comparisonCount }}</span>
          </NuxtLink>
          ...
          <NuxtLink to="/wishlist" class="flex items-center text-luxury-green hover:text-gold transition-colors duration-300 py-2 dark:text-gray-200 dark:hover:text-gold relative">
            ...
            Wishlist
            <span v-if="wishlistCount > 0" class="absolute -top-2 -right-2 bg-gold text-luxury-green rounded-full w-5 h-5 flex items-center justify-center text-xs font-bold animate-pop-in dark:bg-luxury-green dark:text-white">{{ wishlistCount }}</span>
          </NuxtLink>
...
// Get wishlist count
const wishlistCount = computed(() => wishlist.getItemCount.value || wishlist.getItemCount || 0)

// Get comparison count
const comparisonCount = computed(() => getComparisonCount())
